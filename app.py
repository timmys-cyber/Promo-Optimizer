...
if run_scan:
    ...
    if not api_key:
        ...
        # Scan up to 5 days ahead
        scan_limit = (now_utc + timedelta(days=5)).strftime('%Y-%m-%dT%H:%M:%SZ')
        # Buffer to catch games that just started
        ...
        if not all_opps:
            st.warning("No matches found. Check if lines are posted for today's Olympic slate.")
        else:
